...
<script>
    export let owner;
    import {API , PROFIMG} from "../../config"
...
async function deleteUser() {
    const response = await fetch(`${API}/admin/deleteuser/${owner}`, {
            method: "PUT",
            headers: {
                "Content-Type": "application/json"
            }
...
<div class="containerr bg-lite">
    <div class="userdetails">
        ...
        <div class="prof-div"><img class="img-fluid" src={`${PROFIMG}/${userData.profile}`} alt=""></div>
        <h5><span>Name    :</span>{userData.name}</h5>
        <h5><span>E-mail  :</span>{userData.email}</h5>
        <button class="btndanger" on:click={()=>{
            deleteUser()
        }}>
            Delete User
        </button>
                ...
                <form on:submit|preventDefault={()=>{}} >
                    <div class="cover">
                        <div class="form-group mt-2">
                            <input bind:value={userData.email} type="text" class="form-control my-2" id="exampleInputEmail1" aria-describedby="emailHelp"
                                placeholder="Enter email">
